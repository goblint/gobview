...
type data_set =
  | Data of string
  | Set of key_value option
  | Map of key_value list
...
type call =
  | Call of c_id * c_file * c_line * c_order * (context * path option) list
...
let rec parse_data_set c =
  match X.tag c with
  | "data" ->
      Data
        ( X.children c
        |> fun x -> List.nth_opt x 0 |> Option.map X.pcdata |> default "" )
  | "set" ->
      Set
        (X.children c |> fun x -> List.nth_opt x 0 |> Option.map parse_key_value)
  | "map" ->
      Map (X.children c |> List.map parse_key_value)
  | _ ->
      error "Alex expected data_set"

and parse_key_value c =
  match X.tag c with
  | "value" ->
      Value (parse_data_set @@ List.hd @@ X.children c)
  | "key" ->
      Key (X.pcdata @@ List.hd @@ X.children c)
  | _ ->
      error "Alex expected key or value"

let parse_analysis c =
  Analysis (X.attrib c "name", parse_key_value @@ List.hd @@ X.children c)
...
let parse_call c =
  ...
  let context =
    List.map parse_context
    @@ List.find_all (fun x -> X.tag x = "context")
    @@ X.children c
  in
  let path =
    List.map parse_path
    @@ List.find_all (fun x -> X.tag x = "path")
    @@ X.children c
  in
...
let parse_glob c =
  Glob
    ( parse_key_value @@ List.find (fun x -> X.tag x = "key") @@ X.children c
    , List.map parse_analysis
        (List.filter (fun x -> X.tag x = "analysis") @@ X.children c) )
...
let parse_result c =
  ...
  Result
    ...
    , List.map parse_call calls
    , List.map parse_glob globs
    , List.map parse_warning warnings )

let parse_statistics = function
  | Some x ->
      Statistics (List.nth (X.children x) 0 |> X.pcdata)
  | None ->
      Statistics ""

let parse (c : X.xml) : run =
  match X.tag c with
  | "run" ->
      let child = X.children c in
      let parameters =
        List.find (fun x -> X.tag x = "parameters") child |> parse_parameters
      in
      let result =
        List.find (fun x -> X.tag x = "result") child |> parse_result
      in
      let statistics =
        List.find_opt (fun x -> X.tag x = "statistics") child
        |> parse_statistics
      in
      (* let statistics = Statistics "asd" in *)
      Run (parameters, result, statistics)
  | _ ->
      error "Alex expected run tag"
...
let rec list_to_kv_tuple l =
  if List.length l > 0 then
    match l with
    | x :: y :: z ->
        [(x, y)] @ list_to_kv_tuple z
    | _ ->
        error "Alex expected for each key a value in xml map"
  else []

let rec data_set_to_tree = function
  | Data s ->
      T.Node (s, [])
  | Set (Some (Value (Data x))) ->
      T.Node (x, [])
  | Set kv ->
      T.Node ("set", default_app [] (fun x -> [key_value_to_tree x]) kv)
  | Map kvl ->
      T.Node ("map", List.map key_value_tuple_to_tree @@ list_to_kv_tuple kvl)

and key_value_to_tree = function
  | Value (Data x) ->
      T.Node (x, [])
  | Value data ->
      T.Node ("value", [data_set_to_tree data])
  | Key s ->
      T.Node (s, [])

and key_value_tuple_to_tree = function
  | Key s, Value (Set (Some (Value (Data x)))) ->
      T.Node (s ^ " → " ^ x, [])
  | Key s, Value (Data x) ->
      T.Node (s ^ " → " ^ x, [])
  | Key s, Value (Map kvl) ->
      T.Node (s, List.map key_value_tuple_to_tree @@ list_to_kv_tuple kvl)
  | Key s, Value data ->
      T.Node (s, [data_set_to_tree data])
  | _ ->
      error "Alex expected proper key value pairs"

let analysis_to_tree (Analysis (name, value)) =
  match value with
  | Value (Data x) ->
      T.Node (name ^ " → " ^ x, [])
  | Value (Map kvl) ->
      T.Node (name, List.map key_value_tuple_to_tree @@ list_to_kv_tuple kvl)
  | Value (Set None) ->
      T.Node (name ^ " → ∅", [])
  | _ ->
      T.Node (name, [key_value_to_tree value])

let context_to_tree c =
  let (Context analysis_list) = c in
  T.Node ("context", List.map analysis_to_tree analysis_list)

let path_to_tree (Path analysis_list) =
  T.Node ("path", List.map analysis_to_tree analysis_list)

let path_to_some_tree = function None -> [] | Some p -> [path_to_tree p]

let call_to_tree (Call (id, _, _, _, l)) =
  let context_path_list =
    if List.length l > 1 then
      List.mapi
        (fun i (c, p) ->
          T.Node
            ( "Tuple " ^ string_of_int i
            , [context_to_tree c] @ path_to_some_tree p ))
        l
    else
      let c, p = List.hd l in
      [context_to_tree c] @ path_to_some_tree p
  in
...
let get_glob_name = function
  | Key s ->
      s
  | _ ->
      error "Alex expected a Key not Value"

let glob_to_tree (Glob (k, analysis_list)) =
  T.Node (get_glob_name k, List.map analysis_to_tree analysis_list)

let glob_to_something (Glob (name, al)) something =
  match List.find (fun (Analysis (a, _)) -> String.equal a something) al with
  | Analysis (_, Value (Data d)) ->
      T.Node (get_glob_name name ^ " → " ^ d, [])
  | Analysis (_, Value (Set None)) ->
      T.Node (get_glob_name name ^ " → ∅", [])
  | Analysis (_, kv) ->
      T.Node (get_glob_name name, [key_value_to_tree kv])
...
let file_to_name (File (_, attribs)) =
  List.assoc_opt "name" attribs |> default "name missing"

let file_to_path (File (_, attribs)) =
  List.assoc_opt "path" attribs |> default "path missing"

let file_is_empty (File (func_list, _)) =
  let sum =
    List.map (fun (Funct (nl, _)) -> List.length nl) func_list
    |> List.fold_left ( + ) 0
  in
  sum = 0

let get_file_from_filepath s =
  let l = String.split_on_char '/' s in
  List.nth l (List.length l - 1)

let has_dead_code (Call (_, _, _, _, l)) =
  List.length l > 0
  && not
       (List.exists (fun (_, x) -> match x with None -> false | _ -> true) l)

let get_id (Call (id, _, _, _, _)) = id
...
let search_main_file fl =
  let contains_main (File (l, _)) =
    List.exists (fun (Funct (_, name)) -> String.equal name "main") l
  in
...
let glob_to_inverted_tree_2 gl =
  let add_element (l : ('a * 'b) list) (glob_name, glob_value) =
    let found, li =
      List.fold_left
        (fun (found, inner_list) (key, value) ->
          if String.equal key glob_name then
            (true, (key, glob_value :: value) :: inner_list)
          else (found, (key, value) :: inner_list))
        (false, []) l
    in
    if found then li else (glob_name, [glob_value]) :: li
  in
  let add_single_analysis (Analysis (aname, kv)) gname l =
    let x =
      ...
      | Value (Set None) ->
          T.Node (gname ^ " → ∅", [])
      | kv ->
          T.Node (gname, [key_value_to_tree kv])
    in
    add_element l (aname, x)
  in
  let add_glob_to_map l (Glob (gname, al)) =
    let gname = get_glob_name gname in
    List.fold_left (fun s a -> add_single_analysis a gname s) l al
  in
...
let sort_calls_by_line =
  List.sort (fun x y ->
      (get_line x |> int_of_string) - (get_line y |> int_of_string))

let rec calc_difference (Node (s1, t1l) : T.tree) (Node (s2, t2l) : T.tree) :
    T.dtree =
