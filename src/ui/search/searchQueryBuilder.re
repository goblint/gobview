...
let make = (~kind, ~target, ~find, ~structure, ~expression, ~mode, ~dispatch) => {
  ...
  let on_change_expression = e => if(String.trim(e) != ""){dispatch @@ `UpdateSearchField(`Expression(Some(e)))}
    else if (String.trim(e) == "") {dispatch @@ `UpdateSearchField(`Expression(None))};
